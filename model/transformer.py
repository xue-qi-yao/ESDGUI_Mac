...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V):
        ...
        # ln = ln.cuda()
        return ln(output + residual), attn  # All batch size dimensions are reserved.
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ln = nn.LayerNorm(self.d_model)
        # ln = ln.cuda()
        return ln(output + residual)  # [batch_size, seq_len, d_model]
...
class Transformer(nn.Module):
    ...
    def forward(self, x, long_feature):
        ...
        for i in range(out_features.size(1)):
            if i < self.len_q-1:
                input0 = torch.zeros((bs, self.len_q - 1 - i, self.d_model))
                # input0 = input0.cuda()
        ...
        for i in range(feas.size(1)):
            if i < spa_len - 1:
                input0 = torch.zeros((bs, spa_len - 1 - i, 32))
                # input0 = input0.cuda()
